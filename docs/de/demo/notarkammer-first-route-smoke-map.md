...
| `protected_first_matter_status` | `https://app.notariat8.de/workspace/immobilienkaufvertrag` öffnet nur nach bestätigter Sitzung und Bindung; approved session and binding. | Der geschützte Vorgangsstatus bleibt metadata-only und öffnet keinen vollständigen Arbeitsbereich. |
...
| R3 | `https://app.notariat8.de/workspace/immobilienkaufvertrag` als `protected_first_matter_status` öffnen. | Ersten Vorgang über `DEMO-MATTER-IMMOBILIENKAUF-01`, Vorgangstyp `immobilienkaufvertrag` und `bpmn/immobilienkaufvertrag.bpmn` verbinden. | Auf `notarkammer-first-matter-metadata.md` bleiben und metadata-only Scope erklären. |
