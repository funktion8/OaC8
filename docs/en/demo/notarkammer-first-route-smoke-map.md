...
| `first_matter_status` | `https://app.notariat8.de/workspace/immobilienkaufvertrag` shows only protected metadata for the first matter after confirmed session and role. | The matter status is a protected status view; without session, role or binding it stays fail-closed. |
...
| R3 | Open `https://app.notariat8.de/workspace/immobilienkaufvertrag` and explain the first matter through `DEMO-MATTER-IMMOBILIENKAUF-01`. | If session, role and binding are confirmed, connect protected matter metadata, matter type `immobilienkaufvertrag` and `bpmn/immobilienkaufvertrag.bpmn`. | If closed, name `first_matter_status` as the expected fail-closed boundary; otherwise stay on `notarkammer-first-matter-metadata.md` and explain metadata-only scope. |
